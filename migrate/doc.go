...
We only support migrations between mainnet black releases.
We only support migrations from the previous mainnet black version to the current. We don't support migrating between two old versions, use the old software version for this.
...
eg black-3 migrates `auth.GenesisState` from black-2 to `auth.GenesisState` from black-3,
but for black-4 we don't need to keep around black-2's `auth.GenesisState` type.
