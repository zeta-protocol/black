...
type SigningAccount struct {
	...
	blackSigner *util.BlackSigner
	sdkReqChan chan<- util.BlackMsgRequest
	sdkResChan <-chan util.BlackMsgResponse
...
func (chain *Chain) AddNewSigningAccount(name string, hdPath *hd.BIP44Params, chainId, mnemonic string) *SigningAccount {
	...
	// Black signing account for SDK side
	...
	blackSigner := util.NewBlackSigner(
		chainId,
	...
	sdkReqChan := make(chan util.BlackMsgRequest)
	sdkResChan, err := blackSigner.Run(sdkReqChan)
	require.NoErrorf(chain.t, err, "failed to start signer for account %s: %s", name, err)

	// Black signing account for EVM side
	...
	chain.accounts[name] = &SigningAccount{
		...
		SdkAddress: blackSigner.Address(),
	}
...
func (chain *Chain) NewFundedAccount(name string, funds sdk.Coins) *SigningAccount {
	...
	res := whale.SignAndBroadcastBlackTx(
		util.BlackMsgRequest{
			Msgs: []sdk.Msg{
