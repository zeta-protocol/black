...
// InitBlackEvmData is run after the chain is running, but before the tests are run.
// It is used to initialize some EVM state, such as deploying contracts.
func (suite *E2eTestSuite) InitBlackEvmData() {
	whale := suite.Black.GetAccount(FundedAccountName)
	...
	suite.Black.ContractAddrs["greeter"] = greeterAddr
}
...
func (suite *E2eTestSuite) FundBlackErc20Balance(toAddress common.Address, amount *big.Int) EvmTxResponse {
	...
	nonce, err := suite.Black.EvmClient.PendingNonceAt(context.Background(), whale.EvmAddress)
	suite.NoError(err)
...
func (suite *E2eTestSuite) GetErc20Balance(address common.Address) *big.Int {
	resData, err := suite.Black.EvmClient.CallContract(context.Background(), ethereum.CallMsg{
