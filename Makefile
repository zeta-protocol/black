...
###                             Project Info                                 ###
################################################################################
PROJECT_NAME := black# unique namespace for project
...
HTTPS_GIT := https://github.com/Zeta-Protocol/black.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=black \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=black \
...
build: go.sum
ifeq ($(OS), Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o out/$(shell go env GOOS)/black.exe ./cmd/black
else
	go build -mod=readonly $(BUILD_FLAGS) -o out/$(shell go env GOOS)/black ./cmd/black
endif
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/black
...
link-check:
	...
	liche -r . --exclude "^http://127.*|^https://riot.im/app*|^http://black-testnet*|^https://testnet-dex*|^https://black3.data.black.io*|^https://ipfs.io*|^https://apps.apple.com*|^https://black.quicksync.io*"
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/zeta-protocol/black
.PHONY: format
...
# Build docker image and tag as black/black:local
docker-build:
	DOCKER_BUILDKIT=1 $(DOCKER) build -t black/black:local .

build-docker-local-black:
	@$(MAKE) -C networks/local

# Run a 4-node testnet locally
localnet-start: build-linux localnet-stop
	@if ! [ -f build/node0/bkd/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/bkd:Z black/blacknode testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
...
test-e2e: docker-build
	export E2E_BLACK_FUNDED_ACCOUNT_MNEMONIC='tent fitness boat among census primary pipe nose dream glance cave turtle electric fabric jacket shaft easy myself genuine this sibling pulse word unfold'; \
...
# These tests use the `bkd` or `bkcli` binaries in the build dir, or in `$BUILDDIR` if that env var is set.
test-cli: build
...
start-remote-sims:
	...
	docker build -f simulations/Dockerfile -t black/black-sim:master .
	# push that image to the hub
	docker push black/black-sim:master
	...
	aws batch submit-job \
		...
		-—job-definition black-sim-master \
