...
import (
	...
	"github.com/zeta-protocol/black/x/cdp/types"
)
...
Example:
$ %s query %s cdp black15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw atom-a
...
Example:
$ bkcli q cdp cdps --collateral-type=bnb
$ bkcli q cdp cdps --owner=black1hatdq32u5x4wnxrtv5wzjzmq49sxgjgsj0mffm
$ bkcli q cdp cdps --id=21
$ bkcli q cdp cdps --ratio=2.75
$ bkcli q cdp cdps --page=2 --limit=100
`,
		),
		RunE: func(cmd *cobra.Command, args []string) error {
...
Example:
$ %s query %s deposits black15qdefkmwswysgg4qxgqpqr35k3m49pkx2jdfnw atom-a
`, version.AppName, types.ModuleName)),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
