...
func (suite *PayoutTestSuite) TestSendCoinsToBaseAccount() {
	authBuilder := app.NewAuthBankGenesisBuilder().
		...
		WithSimpleModuleAccount(blackdisttypes.ModuleName, cs(c("ufury", 600)))
	...
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, blackdisttypes.ModuleName, suite.addrs[1], cs(c("ufury", 100)), 5)
	suite.Require().NoError(err)
...
func (suite *PayoutTestSuite) TestSendCoinsToInvalidAccount() {
	...
	err := suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, blackdisttypes.ModuleName, suite.addrs[2], cs(c("ufury", 100)), 5)
	suite.Require().ErrorIs(err, types.ErrAccountNotFound)

	macc := suite.getModuleAccount(cdptypes.ModuleName)
	err = suite.keeper.SendTimeLockedCoinsToAccount(suite.ctx, blackdisttypes.ModuleName, macc.GetAddress(), cs(c("ufury", 100)), 5)
