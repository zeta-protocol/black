...
func (suite *KeeperTestSuite) TestBurnDerivative() {
	...
	testCases := []struct {
		...
		},
		{
			...
			balance:          c(liquidDenom, 1e9),
			moduleDelegation: i(1e9),
			burnAmount:       c(fmt.Sprintf("cblack-%s", valAddr), 1e6),
...
func (suite *KeeperTestSuite) TestGetStakedTokensForDerivatives() {
	...
	testCases := []struct {
		...
		},
		{
			...
			// vestedBalance * 95%
			wantBlackAmount: vestedBalance.Mul(sdkmath.NewInt(95)).Quo(sdkmath.NewInt(100)),
		},
		{
			name: "valid - sum",
			derivatives: sdk.NewCoins(
			...
			wantBlackAmount: vestedBalance.Mul(sdkmath.NewInt(95)).Quo(sdkmath.NewInt(100)).Add(vestedBalance),
		},
		{
		...
		},
		{
			name: "invalid - denom",
			derivatives: sdk.NewCoins(
				sdk.NewCoin("black", vestedBalance),
			),
			err: fmt.Errorf("invalid derivative denom: cannot parse denom black"),
		},
	}

	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			blackAmount, err := suite.Keeper.GetStakedTokensForDerivatives(suite.Ctx, tc.derivatives)

			if tc.err != nil {
				suite.Require().Error(err)
			} else {
				suite.Require().NoError(err)
				suite.Require().Equal(suite.NewBondCoin(tc.wantBlackAmount), blackAmount)
			}
		})
	}
}
