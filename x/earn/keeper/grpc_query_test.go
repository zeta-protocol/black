...
func (suite *grpcQueryTestSuite) TestVaults_WithSupply() {
	vaultDenom := "usdx"
	vault2Denom := testutil.TestBblackDenoms[0]

	depositAmount := sdk.NewInt64Coin(vaultDenom, 100)
	deposit2Amount := sdk.NewInt64Coin(vault2Denom, 100)

	suite.CreateVault(vaultDenom, types.StrategyTypes{types.STRATEGY_TYPE_HARD}, false, nil)
	suite.CreateVault("bblack", types.StrategyTypes{types.STRATEGY_TYPE_SAVINGS}, false, nil)
...
func (suite *grpcQueryTestSuite) TestVaults_MixedSupply() {
	vaultDenom := "usdx"
	vault2Denom := "busd"
	vault3Denom := testutil.TestBblackDenoms[0]
...
func (suite *grpcQueryTestSuite) TestDeposits() {
	...
	vault1Denom := "usdx"
	vault2Denom := "busd"
	vault3Denom := fmt.Sprintf("bblack-%s", valAddr1.String())
	vault4Denom := fmt.Sprintf("bblack-%s", valAddr2.String())
	...
	suite.CreateNewUnbondedValidator(valAddr1, initialUblackBalance)
	suite.CreateNewUnbondedValidator(valAddr2, initialUblackBalance)
	...
	savingsParams.SupportedDenoms = append(savingsParams.SupportedDenoms, "bblack")
	...
	suite.Run("specific bblack vault", func() {
		res, err := suite.queryClient.Deposits(
			context.Background(),
	...
	suite.Run("specific bblack vault in staked tokens", func() {
		res, err := suite.queryClient.Deposits(
			context.Background(),
...
func (suite *grpcQueryTestSuite) TestDeposits_bBlack() {
	...
	suite.Run("no deposits", func() {
		...
		res, err := suite.queryClient.Deposits(
			context.Background(),
			types.NewQueryDepositsRequest(address1.String(), "bblack", false, nil),
		)
		suite.Require().NoError(err)
		suite.Require().Len(res.Deposits, 1)
		suite.Require().ElementsMatchf(
			[]types.DepositResponse{
				{
					...
					// Zero shares for "bblack" aggregate
					Shares: nil,
					// Only the specified vault denom value
					Value: nil,
				},
	...
	suite.Run("multiple deposits", func() {
		...
		suite.Require().NoError(err)
		suite.Require().Len(res.Deposits, 1)
		// first validator isn't slashed, so bblack units equal to underlying staked tokens
		// last validator slashed 50% so derivatives are worth half
		// Excludes non-bblack deposits
...
func (suite *grpcQueryTestSuite) TestVault_bBlack_Single() {
	vaultDenom := "bblack"
...
func (suite *grpcQueryTestSuite) TestVault_bBlack_Aggregate() {
	vaultDenom := "bblack"

	address1, derivatives1, _ := suite.createAccountWithDerivatives(testutil.TestBblackDenoms[0], sdkmath.NewInt(1e9))
	address2, derivatives2, _ := suite.createAccountWithDerivatives(testutil.TestBblackDenoms[1], sdkmath.NewInt(1e9))
	address3, derivatives3, _ := suite.createAccountWithDerivatives(testutil.TestBblackDenoms[2], sdkmath.NewInt(1e9))
	// Slash the last validator to reduce the value of it's derivatives to test bblack to underlying token conversion.
	// First call end block to bond validator to enable slashing.
	staking.EndBlocker(suite.Ctx, suite.App.GetStakingKeeper())
	...
	// Query "bblack" to get aggregate amount
	res, err := suite.queryClient.Vault(
		context.Background(),
		types.NewQueryVaultRequest(vaultDenom),
	)
	suite.Require().NoError(err)
	// first two validators are not slashed, so bblack units equal to underlying staked tokens
...
func (suite *grpcQueryTestSuite) TestTotalSupply() {
	...
	testCases := []struct {
		...
		{
			name: "aggregates supply of bblack vaults accounting for slashing",
			setup: func() {
				address1, derivatives1, _ := suite.createAccountWithDerivatives(testutil.TestBblackDenoms[0], sdkmath.NewInt(1e9))
				address2, derivatives2, _ := suite.createAccountWithDerivatives(testutil.TestBblackDenoms[1], sdkmath.NewInt(1e9))
				...
				// create "bblack" vault. it holds all bblack denoms
				suite.CreateVault("bblack", types.StrategyTypes{types.STRATEGY_TYPE_SAVINGS}, false, []sdk.AccAddress{})

				// deposit bblack
				deposit(address1, testutil.TestBblackDenoms[0], derivatives1.Amount.Int64())
				deposit(address2, testutil.TestBblackDenoms[1], derivatives2.Amount.Int64())
			},
			expectedSupply: sdk.NewCoins(
				sdk.NewCoin(
					"bblack",
