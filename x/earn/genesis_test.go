...
import (
	...
	"github.com/zeta-protocol/black/x/earn"
...
func (suite *genesisTestSuite) Test_InitAndExportGenesis() {
	depositor_1, err := sdk.AccAddressFromBech32("black1esagqd83rhqdtpy5sxhklaxgn58k2m3s3mnpea")
	suite.Require().NoError(err)
	depositor_2, err := sdk.AccAddressFromBech32("black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w")
	suite.Require().NoError(err)
	...
	state := types.NewGenesisState(
		...
		types.VaultRecords{
			types.VaultRecord{
				TotalShares: types.NewVaultShare("ublack", sdk.NewDec(3800000)),
			},
			types.VaultRecord{
		...
		types.VaultShareRecords{
			types.VaultShareRecord{
				...
				Shares: types.NewVaultShares(
					...
					types.NewVaultShare("ublack", sdk.NewDec(1900000)),
				),
			},
			types.VaultShareRecord{
	...
	vaultRecord1, _ := suite.Keeper.GetVaultRecord(suite.Ctx, "ublack")
