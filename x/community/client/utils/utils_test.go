...
`)
	...
	require.Equal(t, "Deposit some BLACK from community pool to Lend!", proposal.Description)
...
`)
	...
	require.Equal(t, "Withdraw some BLACK from community pool to Lend!", proposal.Description)
	require.Equal(t, expectedAmount, proposal.Amount)
}
