...
import (
	...
	"github.com/zeta-protocol/black/x/community/client/utils"
	"github.com/zeta-protocol/black/x/community/types"
)
...
func getCmdFundCommunityPool() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(
			`%s tx %s fund-community-module 10000000ublack --from <key>`, version.AppName, types.ModuleName,
		),
...
Example:
$ %s tx gov submit-proposal community-pool-lend-deposit <path/to/proposal.json> --deposit 1000000000ublack --from=<key_or_address>
Where proposal.json contains:
{
  "title": "Community Pool Deposit",
  "description": "Deposit some BLACK from community pool!",
  "amount": [
    {
      "denom": "ublack",
      "amount": "100000000000"
...
Note that --deposit below is the initial proposal deposit submitted along with the proposal.
Example:
$ %s tx gov submit-proposal community-pool-lend-withdraw <path/to/proposal.json> --deposit 1000000000ublack --from=<key_or_address>
Where proposal.json contains:
{
  "title": "Community Pool Withdrawal",
  "description": "Withdraw some BLACK from community pool!",
  "amount": [
    {
      "denom": "ublack",
      "amount": "100000000000"
