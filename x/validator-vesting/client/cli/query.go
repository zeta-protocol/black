...
import (
	...
	"github.com/zeta-protocol/black/x/validator-vesting/types"
)

// GetQueryCmd returns the cli query commands for the blackdist module
...
func queryCirculatingSupply() *cobra.Command {
	return &cobra.Command{
		...
		Long:  "Get the current circulating supply of black tokens",
		Args:  cobra.NoArgs,
...
func queryTotalSupply() *cobra.Command {
	return &cobra.Command{
		...
		Long:  "Get the current total supply of black tokens",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
