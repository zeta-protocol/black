...
func ParseOrQueryConversionPairAddress(
	...
	if err := sdk.ValidateDenom(addrOrDenom); err != nil {
		return common.Address{}, fmt.Errorf(
			"Black ERC20 '%s' is not a valid hex address or denom",
			addrOrDenom,
		)
	}

	// Valid denom, try looking up as denom to get corresponding Black ERC20 address
	...
	return common.Address{}, fmt.Errorf(
		"Black ERC20 '%s' is not a valid hex address or denom (did not match any denoms in queried enabled conversion pairs)",
		addrOrDenom,
	)
}
