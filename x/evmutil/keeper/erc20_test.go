...
func (suite *ERC20TestSuite) TestDeployBlackWrappedCosmosCoinERC20Contract() {
	suite.Run("fails to deploy invalid contract", func() {
		...
		_, err := suite.Keeper.DeployBlackWrappedCosmosCoinERC20Contract(suite.Ctx, invalidToken)
	...
	suite.Run("deploys contract with expected metadata & permissions", func() {
		...
		addr, err := suite.Keeper.DeployBlackWrappedCosmosCoinERC20Contract(suite.Ctx, token)
		suite.NoError(err)
		suite.NotNil(addr)

		callContract := func(method string, args ...interface{}) ([]interface{}, error) {
			return suite.QueryContract(
				types.ERC20BlackWrappedCosmosCoinContract.ABI,
				caller,
				privKey,
				addr,
				method,
