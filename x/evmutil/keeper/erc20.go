...
// DeployBlackWrappedCosmosCoinERC20Contract validates token details and then deploys an ERC20
...
func (k Keeper) DeployBlackWrappedCosmosCoinERC20Contract(
	ctx sdk.Context,
	...
	packedAbi, err := types.ERC20BlackWrappedCosmosCoinContract.ABI.Pack(
		"", // Empty string for contract constructor
		token.Name,
		token.Symbol,
	...
	data := make([]byte, len(types.ERC20BlackWrappedCosmosCoinContract.Bin)+len(packedAbi))
	copy(
		data[:len(types.ERC20BlackWrappedCosmosCoinContract.Bin)],
		types.ERC20BlackWrappedCosmosCoinContract.Bin,
	)
	copy(
		data[len(types.ERC20BlackWrappedCosmosCoinContract.Bin):],
		packedAbi,
...
func (k *Keeper) GetOrDeployCosmosCoinERC20Contract(
	...
	contractAddress, err := k.DeployBlackWrappedCosmosCoinERC20Contract(ctx, tokenInfo)
	if err != nil {
