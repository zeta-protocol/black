...
// GetBalance returns the total balance of ablack for a given account by address.
...
// SetBalance sets the total balance of ablack for a given account by address.
...
// SendBalance transfers the ablack balance from sender addr to recipient addr.
func (k Keeper) SendBalance(ctx sdk.Context, senderAddr sdk.AccAddress, recipientAddr sdk.AccAddress, amt sdkmath.Int) error {
	if amt.IsNegative() {
		return fmt.Errorf("cannot send a negative amount of ablack: %d", amt)
	}
...
// AddBalance increments the ablack balance of an address.
...
// RemoveBalance decrements the ablack balance of an address.
...
// SetDeployedCosmosCoinContract stores a single deployed ERC20BlackWrappedCosmosCoin contract address
...
// SetDeployedCosmosCoinContract gets a deployed ERC20BlackWrappedCosmosCoin contract address by cosmos denom
