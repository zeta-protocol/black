	...
	tests := []struct {
	...
	}{
		{
			...
			types.ConversionPair{
				BlackERC20Address: testutil.MustNewInternalEVMAddressFromString("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2").Bytes(),
				Denom:            "weth",
			},
			errArgs{
				expectPass: true,
...
func TestConversionPair_GetAddress(t *testing.T) {
	...
	require.Equal(t, types.HexBytes(addr.Bytes()), pair.BlackERC20Address, "struct address should match input bytes")
...
func TestConversionPairs_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			errArgs{
				expectPass: false,
				contains:   "found duplicate enabled conversion pair denom black",
			},
		},
		{
...
func TestAllowedCosmosCoinERC20Token_Validate(t *testing.T) {
	testCases := []struct {
		...
		{
			name:   "valid token",
			token:  types.NewAllowedCosmosCoinERC20Token("uatom", "Black-wrapped ATOM", "kATOM", 6),
			expErr: "",
		},
		{
			name:   "valid - highest allowed decimals",
			token:  types.NewAllowedCosmosCoinERC20Token("uatom", "Black-wrapped ATOM", "kATOM", 255),
			expErr: "",
		},
		{
		...
		},
		{
			...
			token:  types.NewAllowedCosmosCoinERC20Token("uatom", "Black-wrapped ATOM", "kATOM", 256),
