...
func init() {
	...
	err = json.Unmarshal(ERC20BlackWrappedCosmosCoinJSON, &ERC20BlackWrappedCosmosCoinContract)
	if err != nil {
		panic(fmt.Sprintf("failed to unmarshal ERC20BlackWrappedCosmosCoinJSON: %s. %s", err, string(ERC20BlackWrappedCosmosCoinJSON)))
	}

	if len(ERC20BlackWrappedCosmosCoinContract.Bin) == 0 {
		panic("loading ERC20BlackWrappedCosmosCoin contract failed")
	}
}
