...
func TestMsgConvertCoinToERC20(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"valid",
			"black123fxg0l602etulhhcdm0vt7l57qya5wjcrwhzz",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			msg := types.MsgConvertERC20ToCoin{
				...
				BlackERC20Address: tc.contractAddr,
				Amount:           tc.amount,
			}
			err := msg.ValidateBasic()
...
func TestConvertCosmosCoinToERC20_ValidateBasic(t *testing.T) {
	validBlackAddr := app.RandomAddress()
	...
	testCases := []struct {
		...
		{
			name:        "valid",
			initiator:   validBlackAddr.String(),
			receiver:    validHexAddr.String(),
			amount:      validAmount,
			expectedErr: "",
		},
		{
			name:        "invalid - sending to black addr",
			initiator:   validBlackAddr.String(),
