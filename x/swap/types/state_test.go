...
import (
	...
	types "github.com/zeta-protocol/black/x/swap/types"
...
func TestState_NewPoolRecord(t *testing.T) {
	reserves := sdk.NewCoins(usdx(50e6), ublack(10e6))
	...
	assert.PanicsWithValue(t, "reserves must have two denominations", func() {
		reserves := sdk.NewCoins(ublack(10e6))
		_ = types.NewPoolRecord(reserves, totalShares)
	}, "expected panic with 1 coin in reserves")

	assert.PanicsWithValue(t, "reserves must have two denominations", func() {
		reserves := sdk.NewCoins(ublack(10e6), hard(1e6), usdx(20e6))
		_ = types.NewPoolRecord(reserves, totalShares)
...
func TestState_NewPoolRecordFromPool(t *testing.T) {
	...
	assert.Equal(t, types.PoolID("ublack", "usdx"), record.PoolID)
	assert.Equal(t, ublack(10e6), record.ReservesA)
	assert.Equal(t, record.ReservesB, usdx(50e6))
	assert.Equal(t, pool.TotalShares(), record.TotalShares)
	assert.Equal(t, sdk.NewCoins(ublack(10e6), usdx(50e6)), record.Reserves())
...
func TestState_PoolRecord_JSONEncoding(t *testing.T) {
	raw := `{
		"pool_id": "ublack:usdx",
		"reserves_a": { "denom": "ublack", "amount": "1000000" },
	...
	assert.Equal(t, types.PoolID("ublack", "usdx"), record.PoolID)
	assert.Equal(t, ublack(1e6), record.ReservesA)
...
total_shares: "3000000"
`
	record := types.NewPoolRecord(sdk.NewCoins(ublack(1e6), usdx(5e6)), i(3e6))
...
func TestState_PoolRecord_Validations(t *testing.T) {
	validRecord := types.NewPoolRecord(
		sdk.NewCoins(usdx(500e6), ublack(100e6)),
		i(300e6),
	)
	testCases := []struct {
		name        string
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ublackusdx' is invalid",
		},
		{
		...
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ublack:' is invalid",
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'usdx:ublack' is invalid",
		},
		{
			name:        "poolID has duplicate denoms",
			poolID:      "ublack:ublack",
			reservesA:   validRecord.ReservesA,
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ublack:ublack' is invalid",
		},
		{
			...
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "poolID 'ublack:usdx' does not match reserves",
		},
		{
		...
		},
		{
			...
			poolID:      "ublack:usdx",
			reservesA:   sdk.Coin{Denom: "ublack", Amount: sdkmath.NewInt(-1)},
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ublack:usdx' has invalid reserves: -1ublack",
		},
		{
			name:        "zero reserve a",
			poolID:      "ublack:usdx",
			reservesA:   sdk.Coin{Denom: "ublack", Amount: sdk.ZeroInt()},
			reservesB:   validRecord.ReservesB,
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ublack:usdx' has invalid reserves: 0ublack",
		},
		{
			...
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ublack:usdx' has invalid reserves: -1usdx",
		},
		{
			name:        "zero reserve b",
			poolID:      "ublack:usdx",
			...
			totalShares: validRecord.TotalShares,
			expectedErr: "pool 'ublack:usdx' has invalid reserves: 0usdx",
		},
		{
			...
			expectedErr: "pool 'ublack:usdx' has invalid total shares: -1",
		},
		{
			...
			expectedErr: "pool 'ublack:usdx' has invalid total shares: 0",
		},
	}
...
func TestState_PoolRecord_OrderedReserves(t *testing.T) {
	invalidOrder := types.NewPoolRecord(
		...
		sdk.Coins{usdx(500e6), ublack(100e6)},
		i(300e6),
	)
	...
	validOrder := types.NewPoolRecord(
		// force order to not be sorted
		sdk.Coins{ublack(500e6), usdx(100e6)},
		i(300e6),
	)
	assert.NoError(t, validOrder.Validate())

	record_1 := types.NewPoolRecord(sdk.NewCoins(usdx(500e6), ublack(100e6)), i(300e6))
	record_2 := types.NewPoolRecord(sdk.NewCoins(ublack(100e6), usdx(500e6)), i(300e6))
	// ensure no regresssions in NewCoins ordering
	assert.Equal(t, record_1, record_2)
	assert.Equal(t, types.PoolID("ublack", "usdx"), record_1.PoolID)
	assert.Equal(t, types.PoolID("ublack", "usdx"), record_2.PoolID)
}
...
func TestState_PoolRecords_Validation(t *testing.T) {
	...
	assert.Error(t, err)
	assert.EqualError(t, err, "pool 'ublack:usdx' has invalid total shares: -1")
}
...
func TestState_PoolRecords_ValidateUniquePools(t *testing.T) {
	...
	record_2 := types.NewPoolRecord(
		sdk.NewCoins(usdx(5000e6), ublack(1000e6)),
		i(3000e6),
	)
	...
	assert.EqualError(t, invalidRecords.Validate(), "duplicate poolID 'ublack:usdx'")
}

func TestState_NewShareRecord(t *testing.T) {
	depositor := sdk.AccAddress("some user")
	poolID := types.PoolID("ublack", "usdx")
...
func TestState_ShareRecord_JSONEncoding(t *testing.T) {
	raw := `{
		"depositor": "black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w",
		"pool_id": "ublack:usdx",
	...
	assert.Equal(t, "black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w", record.Depositor.String())
	assert.Equal(t, types.PoolID("ublack", "usdx"), record.PoolID)
	assert.Equal(t, i(3e6), record.SharesOwned)
}

func TestState_ShareRecord_YamlEncoding(t *testing.T) {
	expected := `depositor: black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w
pool_id: ublack:usdx
shares_owned: "3000000"
`
	depositor, err := sdk.AccAddressFromBech32("black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w")
	require.NoError(t, err)

	record := types.NewShareRecord(depositor, "ublack:usdx", i(3e6))
	data, err := yaml.Marshal(record)
	require.NoError(t, err)
...
func TestState_ShareRecord_Validations(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedErr: "depositor 'black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'ublack:usdx' has invalid total shares: -1",
		},
		{
			name:        "zero total shares",
			depositor:   validRecord.Depositor,
			...
			expectedErr: "depositor 'black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'ublack:usdx' has invalid total shares: 0",
		},
	}
...
func TestState_ShareRecords_Validation(t *testing.T) {
	...
	err = records.Validate()
	assert.Error(t, err)
	assert.EqualError(t, err, "depositor 'black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and pool 'hard:usdx' has invalid total shares: -1")
}

func TestState_ShareRecords_ValidateUniqueShareRecords(t *testing.T) {
	depositor_1, err := sdk.AccAddressFromBech32("black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w")
	require.NoError(t, err)

	depositor_2, err := sdk.AccAddressFromBech32("black1esagqd83rhqdtpy5sxhklaxgn58k2m3s3mnpea")
	require.NoError(t, err)

	record_1 := types.NewShareRecord(depositor_1, "ublack:usdx", i(20e6))
	record_2 := types.NewShareRecord(depositor_1, "ublack:usdx", i(10e6))
	record_3 := types.NewShareRecord(depositor_1, "hard:usdx", i(20e6))
	record_4 := types.NewShareRecord(depositor_2, "ublack:usdx", i(20e6))
	...
	assert.EqualError(t, invalidRecords.Validate(), "duplicate depositor 'black1mq9qxlhze029lm0frzw2xr6hem8c3k9ts54w0w' and poolID 'ublack:usdx'")
}
