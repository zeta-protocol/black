...
import (
	...
	"github.com/zeta-protocol/black/x/swap/keeper"
	"github.com/zeta-protocol/black/x/swap/testutil"
...
func (suite *msgServerTestSuite) TestDeposit_ExistingPool() {
	...
	deposit := types.NewMsgDeposit(
		...
		suite.BankKeeper.GetBalance(suite.Ctx, depositor.GetAddress(), "ublack"),
		sdk.MustNewDecFromStr("0.01"),
		time.Now().Add(10*time.Minute).Unix(),
	)
...
func (suite *msgServerTestSuite) TestWithdraw_AllShares() {
	...
	suite.PoolDeleted("ublack", "usdx")
	suite.PoolSharesDeleted(depositor.GetAddress(), "ublack", "usdx")
...
func (suite *msgServerTestSuite) TestWithdraw_PartialShares() {
	...
	minTokenA := sdk.NewCoin("ublack", sdkmath.NewInt(4999999))
	...
	suite.PoolShareValueEqual(depositor, types.NewAllowedPool("ublack", "usdx"), reserves.Sub(expectedCoinsReceived...))
...
func (suite *msgServerTestSuite) TestWithdraw_SlippageFailure() {
	...
	minTokenA := sdk.NewCoin("ublack", sdkmath.NewInt(5e6))
...
func (suite *msgServerTestSuite) TestSwapExactForTokens() {
	...
	swapInput := sdk.NewCoin("ublack", sdkmath.NewInt(1e6))
	...
	suite.EventsContains(suite.GetEvents(), sdk.NewEvent(
		...
		sdk.NewAttribute(types.AttributeKeyPoolID, types.PoolID("ublack", "usdx")),
		...
		sdk.NewAttribute(types.AttributeKeyFeePaid, "3000ublack"),
...
func (suite *msgServerTestSuite) TestSwapForExactTokens() {
	...
	expectedSwapInput := sdk.NewCoin("ublack", sdkmath.NewInt(1004015))
	...
	suite.EventsContains(suite.GetEvents(), sdk.NewEvent(
		...
		sdk.NewAttribute(types.AttributeKeyFeePaid, "3013ublack"),
...
func (suite *msgServerTestSuite) TestSwapForExactTokens_SlippageFailure() {
	...
	swapMsg := types.NewMsgSwapForExactTokens(
		requester.GetAddress().String(),
		sdk.NewCoin("ublack", sdkmath.NewInt(990991)),
		swapOutput,
		sdk.MustNewDecFromStr("0.01"),
		time.Now().Add(10*time.Minute).Unix(),
	)
