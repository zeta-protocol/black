...
# fund 5k black to x/community account
black tx community fund-community-pool 5000000000ublack --from dev-wallet --gas-prices 0.5ublack -y
...
black tx gov submit-legacy-proposal --deposit 1000000000ublack --type "Text" --title "Example Proposal" --description "This is an example proposal" --gas auto --gas-adjustment 1.2 --from dev-wallet --gas-prices 0.01ublack -y

# setup god's wallet
echo "${BLACK_TESTNET_GOD_MNEMONIC}" | black keys add --recover god

# create template string for the proposal we want to enact
# https://zeta-protocol.atlassian.net/wiki/spaces/ENG/pages/1228537857/Submitting+Governance+Proposals+WIP
